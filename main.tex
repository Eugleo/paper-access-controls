...
\begin{figure}[t]
  ...
  \begin{center}
    ...
    \caption{
      The user asks a question from the grey zone: one that could be either harmless or harmful, depending on its real-world context.
...
These methods could be modified to incorporate external contextual information, e.g. by giving the model access to the user's ID or institutional affiliation.
Although this makes sense exploring, it would likely result in a more opaque and less modular system than making similar modifications to post-processing methods.
...
\begin{table}[t]
  \caption{Example biology content categories.}
  \vspace{0.1in}
...
\Cref{figure:main} illustrates how this approach might work when a user asks about viral surface proteins.
The model would activate its virology expert module, and an external system would observe this activation, check the user's credentials, and decide whether to allow the response.
This method could approximate the benefits of physically separated models while avoiding the overhead.
