...
\begin{abstract}
  ...
  While only a high-level proposal, our work makes the first step toward enabling granular AI governance that benefits both sides of the utility-safety Pareto frontier: users would gain access to more capabilities without sacrificing safety, and regulators would achieve better oversight without stifling innovation.
\end{abstract}
...
Future implementations of such frameworks could enable more nuanced regulatory approaches, with policies that differentiate between user types and contexts rather than forcing blanket restrictions on model capabilities.
This granular approach would benefit both sides of the utility-safety tradeoff: users would gain access to more capabilities without sacrificing safety, and regulators would achieve better oversight without stifling innovation.
...
% TODO Keeping everything else fixed and improving jailbreaking resilience or resilience to prompt injections won't help us, because if we reject the question, SOMETIMES that will be right, but SOMETIMES that will be wrong. We NEED the context.
