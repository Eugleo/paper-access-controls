...
\begin{abstract}
  ...
  The only way to know is to look at their real-world context, but current safety systems do not do that.
  Instead, they make arbitrary decisions that hurt both the total utility and total safety: they refuse some legitimate queries, and fail to refuse some harmful ones.
  ...
  We describe the framework's components, show it is feasible to implement, and explain how it addresses both over-refusals and under-refusals.
...
Safety systems that rely solely on content immediately face the \emph{dual-use dilemma}: when confronted with a grey-zone request, should they refuse it or not?
...
Some safety systems attempt to address this by inferring real-world context from chat history or user-provided rationales.
However, these context sources are user-supplied and easily fabricated by adversaries.
...
\textbf{First}, we show how this verified context can be used jointly with content analysis in a safety framework based on access controls \cite{butler1974}.
...
\textbf{Second}, we propose a novel theoretical approach to content category classification based on recent methods for robust unlearning, UNDO \cite{lee2025distillationrobustifiesunlearning} and gradient routing \cite{cloud2024gradientroutingmaskinggradients}.
...
It enables a more nuanced regulatory approach, with policies that differentiate between user types and contexts rather than forcing blanket restrictions on model capabilities.
