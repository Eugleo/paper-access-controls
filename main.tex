...
In the previous section, we established that current safety systems cannot make truly context-aware decisions about whether to refuse a request because of a lack of access to trustworthy real-world context.
In this section, we describe a verification-based access control system that does not have this issue.
...
First, they define \textbf{content categories} (\cref{section:risk-categories}): groups of sensitive topics organized by domain and risk rating.
...
The main concern is increased user friction, but we show in \cref{section:feasibility-analysis} that this should be minimal; among other things, because most users will never make grey-zone requests.

% TODO Maybe make a figure that describes the 4 parts and make it be figure 1.

\subsection{Risk Categories} \label{section:risk-categories}

% TODO Maybe add table that illustrates that these are both horizontal and vertical.

As with verification levels, we expect model providers will consult domain experts to find the right risk categories for their models.
These will likely be based on existing domain-specific risk categories for physical goods (chemicals, organisms, equipment), first adapted to the field of knowledge (e.g. regulating the distribution of equipment will turn to regulating the knowledge about how to operate said equipment correctly), and later iteratively refined based on real usage data, incident reports, etc.
We expect different fields of knowledge to end up with 1--4 risk categories.

Each risk category is actually a collection of topics associated to a concrete verification level.
Whenever the system detects that the model is generating an output from one of these topics, it will check whether the user has the appropriate verification level.
Thus, the main desiderata for risk categories are:
\begin{itemize}
  \item Feasibility of classification: Risk categories should be technically feasible to classify solely based on information we have access to during a model's forward pass. Good example: ``safety protocols to follow in a BSL-3 laboratory''. Bad example: ``how to use CRISPR to insert dangerous genes'' (CRISPR works the same for all genes, so the model cannot distinguish between dangerous and harmless applications of CRISPR). We address different ways to do this classification in \cref{section:content-classification}.
  \item Balancing friction and safety: High-tier categories should be used sparingly, as high-tier verification will add a lot of friction to users. A good heuristic would be to go through core, isolated concepts in a field (isolated meaning they are classifiable according to the definition above), such as operating a concrete piece of equipment, working with a specific organism, or performing specific kinds of experiments (e.g. anything related to membrane protein modifications) with multiple experts and have them say how often the given knowledge is used in harmful ways vs. how often it is used in harmless ways. Thus, it should be a low-tier risk category. We talk more about friction in \cref{section:feasibility-analysis}.
\end{itemize}

To continue with our running example from virology: CRISPR is potentially dangerous, but it is also used in a lot of harmless ways, as it is a very widely spread technology.
Thus, it should be a low-tier risk category, to balance the risk of misuse against user friction --- e.g. one that only requires a basic ID-based verification or institutional affiliation.
Technologies that are used for aerosolization, on the other hand, are more dangerous, as they are directly useful for construction of biological weapons.
They are also used in pharmaceutical applications, but we can expect the ratio to be much more balanced between the harmful and the harmless applications.
Thus, they should be a high-tier risk category, e.g. one linked to real-world BSL-3 or BSL-4 certifications.

\subsection{Verification Mechanisms and Credentialling Systems} \label{section:verification-mechanisms}

Developers will leverage existing verification infrastructure, as they do not have the expertise to build their own.
...
For example, access to information about aerosolization techniques of biological agents could require government certification for biosafety level 3 (BSL-3), while access to information about membrane protein modifications might require BSL-4.
...
Governance and decision-making processes would initially remain company-internal, with developers balancing user friction against safety benefits based on usage data and incident reports.
Companies could refine their systems through partial rollouts and iterative testing before full deployment.
While no external oversight would exist initially, we anticipate that successful implementations would eventually inform industry standards or safety certification frameworks as the regulatory landscape matures.
For user disputes regarding credential rejections or risk category assignments, companies would need to establish manual appeals processes, though we expect these to be infrequent given that most users will never encounter any verification requirements.
If the approach proves effective, decision-making authority may shift toward industry consortiums or standardization organizations that can coordinate risk categories and verification levels across providers, similar to existing content moderation or KYC standards in other industries.
